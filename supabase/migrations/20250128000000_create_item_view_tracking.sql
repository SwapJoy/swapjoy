-- Create item view tracking system
-- This migration creates tables and functions for tracking item views
-- and displaying mostly viewed items

SET search_path = public;

-- ============================================================================
-- 1. Create item_metrics table
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_item_metrics_last_viewed_at ON item_metrics(last_viewed_at DESC);

-- ============================================================================
-- 2. Create item_views table (for unique view tracking)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS item_views (
  ...
  -- Unique constraint: one view per user per item (within 30 days)
  -- This is enforced by the tracking function, not a DB constraint
  -- since we want to allow re-views after 30 days
...
-- ============================================================================
-- 3. Trigger to auto-create item_metrics when item is created
-- ============================================================================
...
-- ============================================================================
-- 4. Function to track item view (idempotent per user)
-- ============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- We could use session_id or IP in the future, but for now just count all
    INSERT INTO item_views (item_id, user_id, viewed_at)
...
COMMENT ON FUNCTION fn_track_item_view IS 'Tracks a view for an item. For authenticated users, counts once per 30 days. For anonymous users, counts all views.';

-- ============================================================================
-- 5. Function to get mostly viewed items
-- ============================================================================
...
-- ============================================================================
-- 6. Create metrics for existing items (backfill)
-- ============================================================================
...
WHERE status = 'available'
  AND id NOT IN (SELECT item_id FROM item_metrics)
ON CONFLICT (item_id) DO NOTHING;
