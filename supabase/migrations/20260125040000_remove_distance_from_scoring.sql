-- Remove distance from scoring - distance should only be used for filtering, not sorting
-- Update get_top_picks_for_user to score items based only on similarity and recency
...
blended AS (
  SELECT
    *,
    -- Weighted blend: similarity + recency (NO distance in scoring)
    CASE
      WHEN (SELECT preference_embedding FROM user_pref) IS NOT NULL
      THEN (0.7 * similarity_score + 0.3 * recency_score)  -- 70% similarity, 30% recency when personalized
      ELSE recency_score  -- Just recency for cold start (newest first)
    END AS score
  FROM scored
)
SELECT
