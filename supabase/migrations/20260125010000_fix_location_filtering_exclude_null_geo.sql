-- Fix location filtering to exclude items without geo when location is provided
-- The filtered CTE was allowing items with distance_km IS NULL even when user has location
...
base AS (
  ...
  WHERE i.status = 'available'
    ...
    -- Only include items within radius when user location is provided
    AND (
      -- Case 1: User has no location or no radius -> show all items (including those without geo)
      (p_user_lat IS NULL OR p_user_lng IS NULL OR p_radius_km IS NULL)
      OR
      -- Case 2: User has location and radius -> only show items with geo that are within radius
...
filtered AS (
  SELECT *
  FROM base
  -- When user has location and radius, exclude items without geo (distance_km IS NULL)
  WHERE (
    -- If user has no location/radius, show all items (including those without geo)
    (p_user_lat IS NULL OR p_user_lng IS NULL OR p_radius_km IS NULL)
    OR
    -- If user has location/radius, only show items with valid distance within radius
    (p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL AND p_radius_km IS NOT NULL
